  ...
  return (
    <nav
      className={`absolute left-0 w-full flex flex-row items-center z-30
                ${display === "mobile" ? "bottom-0" : "top-0"}`}
    >
      {display === "mobile" ? (
        <div
          id="mobileHeader"
          onClick={logoClick}
          className="fixed top-0 left-0 w-full flex flex-row items-center pt-3 pb-1 px-3 bg-black"
        >
        ...
        <NavLogo
          image={logo}
          onClick={logoClick}
          className="lg:w-44 xl:w-48 2xl:w-52 m-3"
        />
      )}
      ...
      <div
        ...
        className={`flex flex-row justify-center items-center w-full ${
          display === "mobile"
            ? "absolute bottom-0 left-0 pt-1 bg-black"
            : "mx-5"
        }`}
      >
